...
/** Audit: data backup generated (who did it, tables included, format) */
export async function logDataBackup(
  userId: string,
  request: NextRequest,
  payload: { tables: string[]; format: string; rowCounts?: Record<string, number> }
) {
  return logUserActivity({
    userId,
...
/** Audit: schema/table structure export generated (who did it, tables included, format) */
export async function logSchemaExport(
  userId: string,
  request: NextRequest,
  payload: { tables: string[]; format: 'sql' | 'pgsql' }
) {
  return logUserActivity({
    userId,
