...
interface NewPromotionCodeDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function NewPromotionCodeDialog({ open, onOpenChange }: NewPromotionCodeDialogProps) {
  ...
  const onSubmit = async (data: PromotionCodeFormData) => {
    try {
      ...
      if (response.ok) {
        ...
        toast.success(`Promotion code ${result.code} created successfully!`)
        form.reset()
        onOpenChange(false)
        // Trigger a page refresh or update the list
        window.dispatchEvent(new Event('promotion-code-created'))
      } else {
        const error = await response.json()
        toast.error(error.error || 'Failed to create promotion code')
      }
    } catch (error) {
